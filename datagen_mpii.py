...
def img_hm_plot(hm, img):
    from matplotlib import pyplot as plt
    hm5 = scm.imresize(np.repeat(np.sum(hm, axis=2), 3, axis=2), (img.shape[0], img.shape[1]))
    plt.imshow(hm5 * 255 + img.astype(np.float32))
    plt.show()
    plt.savefig('1.png')
...
class DataGenerator(object):
    ...
    def _reduce_joints(self, joints,weightJ):
        """ Select Joints of interest from self.weightJ
        """
        j = []
        zhedang=[]
        for i in range(len(weightJ)):
            if weightJ[i] == 1:
                j.append(joints[i][0])
                j.append(joints[i][1])
                zhedang.append(joints[i][2])
        return j,zhedang

    def _create_train_table(self,cloth=Global.cloth):
        """ Create Table of samples from CSV file
        """
        self.train_table = []
        self.no_intel = []
        self.data_dict = {}
        input_file=csv.reader(open(self.train_data_file,'r'))
        item=input_file.next()
        print('READING TRAIN DATA')
        for item in input_file:
            if item[1] not in cloth:
                continue
            name = item[0] #blouse/图片名
            joints = [[int(t) for t in s.split('_')] for s in item[2:]]
            joints,zhedang=self._reduce_joints(joints,[1]*len(joints))
            if joints == [-1] * len(joints):
                self.no_intel.append(name)
            else:
                joints = np.reshape(joints, (-1, 2))
                w = [1.0] * joints.shape[0]
                for i in range(joints.shape[0]):
                    if np.array_equal(joints[i], [-1, -1]):#没有信息的点不计入考虑范围
                        w[i] = 0.0
                    #if zhedang[i]==0:#遮挡点在计算loss时需要考虑
                       # w[i] = 0.0
                box=[-1]*4;box[2],box[3]=max(joints[:,0]),max(joints[:,1])#这里直接用joints的信息计算出box信息。box[0]和box[1]是左上方点的坐标，box[2]box[3]是右下方坐标
    ...
    def _complete_sample(self, name):
        ...
        for i in range(self.data_dict[name]['joints'].shape[0]):
            if np.array_equal(self.data_dict[name]['joints'][i], [-1, -1]):#不完整的样本只用于训练（权重设为0），不用于测试
                return False
        return True
    ...
    def _makeGaussian(self, height, width, sigma=Global.gaussian_sigma, center=None):
    ...
    def _aux_generator(self, batch_size=16, stacks=4, normalize=True, sample_set='train'):
        ...
        while True:
            ...
            train_gtmap = np.zeros((batch_size, stacks, self.out_size, self.out_size, len(self.joints_list) + 1),np.float32)
            ...
            while i < batch_size:
                ...
                if True:
                    ...
                    #hm5 = scm.imresize(np.sum(hm, axis=2), (self.in_size, self.in_size)); plt.imshow(hm5 * 255 + img4[:, :, 0].astype(np.float32)); plt.show();plt.savefig('1.png')
    ...
    # ---------------------------- Image Reader --------------------------------				
    def open_img(self, name, color='RGB'):#还要加入一个blouse名字
        """ Open an image 
        Args:
            name	: Name of the sample
